...
import addAnnounce from "../../../assets/addAnnounce.svg";
...
export default function AnnouncesList() {
  ...
  let [showAnnounce, setShownAnnounce] = useState({ show: false, id: null });

  let [loading, setLoading] = useState(true);

  let getAllAnnounces = async (page, limit) => {
    try {
      setLoading(true);
      const response = await axios.get(
        "http://localhost:5000/api/v1/annonce/admin/getAllAnnonce?page=" +
          page +
          "&limit=" +
          limit,
        {
  ...
  useEffect(() => {
    document.head.getElementsByTagName("title")[0].innerHTML = "Announcements";
    (async () => {
      let data = await getAllAnnounces(pageSettings.page, pageSettings.limit);
      setUpAnnounces(data);
    })();
  }, []);

  let handleAnnounceClick = (e) => {
    let element = e.target;
    let id = undefined;
    if (!showAnnounce.show) {
      while (element.tagName != "TR") element = element.parentNode;
      id = element.dataset.id;
    }
    setShownAnnounce((prevState) => {
      return { show: !prevState.show, id: id };
    });
  };

  let DisplayAnnounce = () => {
    if (showAnnounce.show && showAnnounce.id) {
      let announceData = announces.filter(
        (announce) => announce.id === showAnnounce.id
      )[0];
      return (
        <AnnounceData
          data={announceData}
  ...
  let [addAnnounceForm, setAddAnnounceForm] = useState(false);

  let toggleAddAnnounceForm = () => {
    setAddAnnounceForm((prev) => !prev);
  };
  ...
  return (
    <section id="mainSection">
      <DisplayAnnounce />
      <DisplayAddAnnounceForm />
      <TopBar />
      <div className="pageList" id="announcesList">
        <header>
          <h2>Announcements</h2>
        ...
        <footer>
          <div className="footerOptions">
            <button id="addAnnounce" onClick={toggleAddAnnounceForm}>
              <img src={addAnnounce} alt="" />
            </button>
          </div>
