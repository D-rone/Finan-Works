...
export default function SubChaptersList() {
  ...
  let getChapter = async (id) => {
    setLoading(true);
    let response = await axios.get(
      "http://localhost:5000/api/v1/chapitre/admin/" + id,
      {
        withCredentials: true,
      }
    );
    let chapter = response?.data?.chapitre;
    setSubChapters(
      chapter?.sous_chapitre.map((data) => {
        return {
          name: data.nom,
    ...
    document.head.getElementsByTagName("title")[0].innerHTML = chapter?.nom;
    setChapterName(chapter?.nom);
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    let chapterId = new URLSearchParams(urlParams).get("chapter");
    if (!chapterId) {
  ...
  let updateSubChaptersStatus = () => {
    let updated = subChapters.filter(
      (sub) =>
        (sub.status == "active" && sub.isHidden == false) ||
        (sub.status != "active" && sub.isHidden == true)
    );
    if (updated.length != 0) {
      console.log(updated);
    }
  ...
  return (
    <section id="mainSection">
      ...
      <div className="pageList" id="subChaptersList">
        <header>
          <h2>
            Sub-Chapters of <b>{chapterName}</b>
          </h2>
        </header>
        <main>
          <div id="tableHeader">
        ...
        <footer>
          <div className="footerOptions">
            <button id="delEmployee" onClick={updateSubChaptersStatus}>
